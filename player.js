...
function injectLangToggle() {
  ...
  `;
  toggle.innerHTML = `
    <button id="btn-sub" class="lang-btn active" style="
      padding: 5px 16px; border-radius: 6px; border: 1px solid var(--accent, #6c63ff);
      background: var(--accent, #6c63ff); color: #fff; cursor: pointer; font-size: 13px; font-weight: 600;
    ">SUB</button>
    <button id="btn-dub" class="lang-btn" style="
      padding: 5px 16px; border-radius: 6px; border: 1px solid var(--accent, #6c63ff);
      background: transparent; color: var(--accent, #6c63ff); cursor: pointer; font-size: 13px; font-weight: 600;
    ">DUB</button>
  `;

  // Insert above the player area
  const playerSection = document.getElementById('player-section');
  if (playerSection) {
    playerSection.insertBefore(toggle, playerSection.firstChild);
  }
...
function switchLang(lang) {
  ...
  document.getElementById('btn-sub').style.background    = lang === 'sub' ? 'var(--accent, #6c63ff)' : 'transparent';
  document.getElementById('btn-sub').style.color         = lang === 'sub' ? '#fff' : 'var(--accent, #6c63ff)';
  document.getElementById('btn-dub').style.background    = lang === 'dub' ? 'var(--accent, #6c63ff)' : 'transparent';
  document.getElementById('btn-dub').style.color         = lang === 'dub' ? '#fff' : 'var(--accent, #6c63ff)';
